...
export async function POST(request: NextRequest) {
	try {
		...
		if (!query) {
			return NextResponse.json({ success: false, error: "缺少查询文本" }, { status: 400 });
		}
		...
		return NextResponse.json({ success: false, error: "搜索失败", details: error instanceof Error ? error.message : String(error) }, { status: 500 });
	}
}
