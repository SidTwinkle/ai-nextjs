...
export async function POST(req: Request) {
	try {
		...
		const userQuery = lastMessage?.parts?.map((part) => (part.type === "text" ? part.text : "")).join("") || "";
...
请基于以上内容回答用户问题。如果文档中没有相关信息，请说明这一点。`;
		...
		return new Response(JSON.stringify({ error: "处理请求时出错", details: error instanceof Error ? error.message : String(error) }), {
			status: 500,
