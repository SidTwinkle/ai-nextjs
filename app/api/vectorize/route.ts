...
export async function POST(request: NextRequest) {
	try {
		...
		try {
			...
			return NextResponse.json({ success: false, error: "文件读取失败" }, { status: 404 });
		}
		...
		// 使用 text-embedding-004 模型（或其他兼容模型）
		const embeddingModel = openai.embeddingModel("text-embedding-004");
		...
		if (vectors.length > 0) {
			...
			return NextResponse.json({ success: false, error: "向量化失败" }, { status: 500 });
		}
	} catch (error) {
		console.error("向量化错误:", error);
		return NextResponse.json({ success: false, error: "向量化处理失败", details: error instanceof Error ? error.message : String(error) }, { status: 500 });
	}
}
